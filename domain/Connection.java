...
public class Connection implements EWrapper {
   // private final JTextArea info;
    ...
    public EClientSocket connect(Consumer consumer) throws InterruptedException {
        final EJavaSignal eReaderSignal = new EJavaSignal();
        EReader reader;
        EClientSocket eClientSocket = new EClientSocket(this, eReaderSignal);
        eClientSocket.eConnect("", 7497, 0);

        //An additional thread is created in this program design to empty the messaging queue
        try {
            while (eClientSocket.isConnected()) {
                final String before_wait = "before wait";
                System.out.println(before_wait);
                consumer.accept(before_wait);

                eReaderSignal.waitForSignal();
                final String after_wait = "after wait";
                System.out.println(after_wait);
                consumer.accept(after_wait);

            }
        } catch (Exception e) {
            final String x = "Exception: " + e.getMessage();
        ...
        reader = new EReader(eClientSocket, eReaderSignal);
        reader.start();
        new Thread() {
            public void run() {
                while (true){
    ...
    public void error(int i, int i1, String s) {
        final String x = "error id: " + i + "errorCode: " + i1 + "error message:" + s;
        System.out.println(x);
